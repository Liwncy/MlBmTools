var globalConfig = storages.create("hallo1_mlbmmidiplayer_config");
const musicDir = "/sdcard/天刀音乐盒数据目录/"
...
dialogs.alert("", "切回游戏，脚本会自动开始(如果发现并未开始，请关掉全局设置中\"等待进入游戏\"选项)")
console.verbose("无障碍服务启动成功");
if (readGlobalConfig("waitForGame", 1)) waitForPackage("com.tencent.tmgp.wuxia");
...
if (device.width == 1440 && device.height == 3120) {
    ...
    var clickx_pos =[574,928,1290,1655,2018,2376,2743];
    var clicky_pos =[1322,1169,1024];
    ...
    if (delaytime0 == delaytime1) { //如果两个音符时间相等，把这个音和后面的一起加入数组
        ...
        for (var j = 0; j < noteList.length; j++) { //遍历这个数组
            ...
            if (tone != 0) {
                ...
                gestureList[gestureList.length] = [0, 1, [clickx_pos[clickx - 1], clicky_pos[clicky - 1]]];
            };
        };
        if (delaytime >= 6) {
            //长音
        ...
        if(delaytime * 1000 - 5>0) sleep(delaytime * 1000 -5);
        noteList = [];
        gestureList = [];
    };
    i++
